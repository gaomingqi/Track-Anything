...
def get_frames_from_video(video_state, interactive_state, mask_dropdown):
    """
    Args:
        video_path:str
        timestamp:float64
    ...
    masks_path.sort()
    first_masks_path = masks_path[0]
    first_masks = np.asarray(Image.open(first_masks_path))

    frames_name = os.listdir(os.path.join(args.davisdir, "JPEGImages/480p", args.sequence))
    frames_name.sort()


    frames = [os.path.join(args.davisdir, "JPEGImages/480p", args.sequence, i) for i in frames_name]
    ...
    video_path = generate_video_from_frames(frames, output_path=os.path.join(args.davisdir, "workspace/frame2video", "{}.mp4".format(args.sequence)))
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    os.system("rm {}".format(os.path.join(args.davisdir, "workspace/result_video", "{}".format(video_state["video_name"]))))
    video_output, video_state = get_mask_from_vot(video_state, output_path=os.path.join(args.davisdir, "workspace/result_video", "{}".format(video_state["video_name"])))
...
with gr.Blocks() as iface:
    ...
    extract_frames_button.click(
        ...
        outputs=[video_input, video_state, video_info, template_frame, image_selection_slider, 
                 ...
                 remove_mask_button, inpaint_video_predict_button, run_status, interactive_state]
    )   
